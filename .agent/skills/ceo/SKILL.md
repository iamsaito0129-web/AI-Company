...
- **`.company/` が存在する場合**: `.company/GEMINI.md` を読み込み、**運営モード**へ
...
```
.company/
├── GEMINI.md                    # 組織全体のルール・設定
├── secretary/                   # 秘書室（常設）
│   ├── GEMINI.md                # 秘書室のルール・口調
│   ├── _template.md
│   ├── inbox/
│   │   └── _template.md
│   ├── todos/
...
3. **各部署の `GEMINI.md` を配置**
   - `references/departments.md` の「部署別 GEMINI.md テンプレート」セクションから取得
   - 部署固有のルール、ステータス遷移、命名規則が定義される

4. **`.company/GEMINI.md` を生成**
...
1. **インボックスチェック (AIブリッジ)**: 
   - 起動時、必ず GitHub の `Issues` をチェックし、`[Mobile Inbox]` または `[Inbox]` ラベルのついた新着項目がないか確認する。
   - 新着があれば、直ちに `secretary/inbox/YYYY-MM-DD.md` へ追記し、ユーザーに「スマホから〇件の入力があります」と報告する。
2. ユーザーが何かを言う
2. 秘書が内容を判断:
   - **秘書で完結するもの** → 秘書が直接対応
...
- GEMINI.md 生成テンプレート: `.agent\skills\company-md-department-templates.md`
...
- 運営モードでは必ず最初に `.company/GEMINI.md` を読み込む
- 部署に振り分ける際は、該当部署の `GEMINI.md` も読み込んでルールに従う
