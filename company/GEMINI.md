...
### 弊社 (AI-Company) と SAC (School-AI-Company) の分離
- **原則**: 弊社（AI-Company）はプライベート、スキルアップ、資産管理、および AI 開発を主戦場とする。
- **分離ルール**: SAC および学校での教諭としての TODO・校務データは、弊社ディレクトリ（`.company/`）内では原則として取り扱わない。
- **例外**: 教材開発のための AI 技術検証など、弊社「開発部」や「リサーチ部」の成果を SAC にフィードバックする場合のみ、境界を明確にして連携する。
